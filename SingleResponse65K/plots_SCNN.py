...
from SCNN import ShallowCNN, val_loader, val_dataset, train_dataset, N_EPOCHS, N, BATCH_SIZE, HIDDEN_LAYER_SIZE, cmap
...
model.load_state_dict(torch.load(f'modelStateDict_{N_EPOCHS}_shallow_final.pth'))
...
plt.suptitle(f'Shallow CNN Validation - Moisture', fontsize=16)
...
#    plt.subplot(4, 4, i+1)
    ...
    plt.title(f'RMSE = {rounded_rmse} | n = {len(pred_vals[mask, i])} \n{HIDDEN_LAYER_SIZE} neurons in hidden layer', loc = 'left')

plt.tight_layout()
plt.savefig(f'figures/SCNN_{N_EPOCHS}_epochs_{HIDDEN_LAYER_SIZE}.png')
