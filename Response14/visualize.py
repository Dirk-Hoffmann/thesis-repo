...
def main(train_dataset, test_dataset, val_dataset, fileID, col):
   import datetime
   ...
   XP, YP = train_dataset.single_feature(col)
   
   print(XP.shape)
   print(YP.shape)


   XP_test, YP_test = test_dataset.single_feature(col)

   XP_val, YP_val = val_dataset.single_feature(col)
  
   # For YP
   valid_row_positions = np.where(~np.isnan(YP))[0]
   YP = YP[valid_row_positions]
   XP = XP[valid_row_positions]

   # For YP_test
   valid_row_positions = np.where(~np.isnan(YP_test))[0]
   YP_test = YP_test[valid_row_positions]
   XP_test = XP_test[valid_row_positions]

   # For YP_val
   valid_row_positions = np.where(~np.isnan(YP_val))[0]
   YP_val = YP_val[valid_row_positions]
   XP_val = XP_val[valid_row_positions]  
   cmap = cmap[valid_row_positions]
...
#      plt.savefig(f'figures/{fileID}_PLS.png')
   ...
   for i, (model_name, model_predictions) in enumerate(models_predictions.items()):
      ...
      print(model_name, model_predictions)
      ...
      print("pred_mean shape:", np.shape(pred_mean))  # Assuming pred_mean is a scalar or a NumPy array
      print("pred_std shape:", np.shape(pred_std))    # Assuming pred_std is a scalar or a NumPy array
   ...
   for key in model_rmse.keys():
      model_rmse[key]=round(float(model_rmse[key]),3)
   return (model_rmse)
