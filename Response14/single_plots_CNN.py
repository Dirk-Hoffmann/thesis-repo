...
from CNN import MultiOutputCNN, val_loader, val_dataset, train_dataset, N_EPOCHS, N, BATCH_SIZE, LEARNING_RATE, cmap
...
for i in range(true_vals.shape[1]):
    ...
    plt.suptitle(f'Deep CNN Validation - {val_dataset.response_names[i]}', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(f'figures/DCNN/individualPlots/CNN_{N_EPOCHS}_epochs_{BATCH_SIZE}_BS_{val_dataset.response_names[i]}.png')
