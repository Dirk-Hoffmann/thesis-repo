...
from SCNN_batchfixed import ShallowCNN, SpectralDataset, val_loader, val_dataset, train_dataset, N_EPOCHS, N, BATCH_SIZE,HIDDEN_LAYER_SIZE
...
model.load_state_dict(torch.load(f'modelStateDict_{N_EPOCHS}_shallow.pth'))
...
for i in range(true_vals.shape[1]):
    ...
    cmap_filtered = cmap[:640][mask]


    plt.figure(figsize=(7, 7))
    plt.suptitle(f'Shallow CNN - {val_dataset.response_names[i]}', fontsize= 16)
    ...
    plt.title(f'RMSE = {rounded_rmse} | n = {len(pred_vals[mask, i])} \nNeurons in Hidden Layer = {HIDDEN_LAYER_SIZE}', loc='left')

    plt.tight_layout()

    plt.savefig(f'figures/SCNN/singles/SCNN_{N_EPOCHS}_epochs_{HIDDEN_LAYER_SIZE}_Neurons_{val_dataset.response_names[i]}.png')
